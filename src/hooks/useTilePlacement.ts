...
export function useTilePlacement({
  ...
  const handleRackTileDrop = useCallback(
    (
      ...
      const debugFail = (reason: string) => {
        console.warn(`[TilePlacement #${placementId}] [${timestamp}] FAILED:`, reason, {
          visualSlot,
          actualRackIndex,
          target,
          ...
          pendingTilePositions: pendingTiles.map(t => `${t.letter}@${t.x},${t.y}`),
        });
        return false;
      };

      console.warn(`[TilePlacement #${placementId}] [${timestamp}] handleRackTileDrop START`, {
        visualSlot,
        actualRackIndex,
        target,
        gameStatus: game?.status,
        pendingTilesBefore: pendingTiles.map(t => `${t.letter}@${t.x},${t.y}`),
      });
      ...
      if (target.type === 'board') {
        ...
        if (tile) {
          console.warn(`[TilePlacement #${placementId}] [${Date.now()}] About to call placeTile`, {
            letter: tile.letter,
            x: target.x,
            y: target.y,
          ...
          const gameState = storeState.currentGameUlid
            ...
            : null;
          console.warn(`[TilePlacement #${placementId}] [${Date.now()}] After placeTile, store state:`, {
            pendingTilesCount: gameState?.pendingTiles.length,
            pendingTiles: gameState?.pendingTiles.map(t => `${t.letter}@${t.x},${t.y}`),
            hasOurTile: gameState?.pendingTiles.some(t => t.x === target.x && t.y === target.y),
          });
          ...
          console.warn(`[TilePlacement #${placementId}] [${Date.now()}] SUCCESS - returning true`, {
        ...
        console.warn(`[TilePlacement #${placementId}] [${Date.now()}] Rack-to-rack swap`);
  ...
  const handlePendingTileDrag = useCallback(
    (fromX: number, fromY: number, target: DropTarget) => {
      const timestamp = Date.now();
      console.warn(`[TilePlacement] [${timestamp}] handlePendingTileDrag called`, {
        fromX,
        fromY,
        target,
        gameStatus: game?.status,
      ...
      if (!game || game.status !== 'active') {
        console.warn(`[TilePlacement] [${timestamp}] handlePendingTileDrag - game inactive, ignoring`);
        return;
      }

      // No target (dropped in empty area) or rack target = return tile to rack
      if (!target || target.type === 'rack') {
        console.warn(`[TilePlacement] [${timestamp}] handlePendingTileDrag - returning to rack (no target or rack target)`);
      ...
      if (target.type === 'board') {
        if (target.x === fromX && target.y === fromY) {
          console.warn(`[TilePlacement] [${timestamp}] handlePendingTileDrag - same position, ignoring`);
          return;
        }
        if (!isBoardPositionAvailable(target.x, target.y)) {
          console.warn(`[TilePlacement] [${timestamp}] handlePendingTileDrag - target occupied, ignoring`);
          return;
        }

        console.warn(`[TilePlacement] [${timestamp}] handlePendingTileDrag - moving tile`);
  ...
  const handleCellPress = useCallback(
    (x: number, y: number) => {
      const timestamp = Date.now();
      console.warn(`[TilePlacement] [${timestamp}] handleCellPress called`, { x, y, gameStatus: game?.status });

      if (!game || game.status !== 'active') {
        console.warn(`[TilePlacement] [${timestamp}] handleCellPress - game inactive, ignoring`);
        return;
      }

      const existing = pendingTiles.find((t) => t.x === x && t.y === y);
      if (existing) {
        console.warn(`[TilePlacement] [${timestamp}] handleCellPress - removing tile at ${x},${y}`);
        removeTile(x, y);
      } else {
        console.warn(`[TilePlacement] [${timestamp}] handleCellPress - no pending tile at ${x},${y}`);
      }
    },
